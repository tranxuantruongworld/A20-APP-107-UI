            ...
            {visibleQuestions.length === 0 ? (
              ...
              visibleQuestions.map((q) => (
                <div key={q.id} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className={`rounded-2xl border-2 p-6 transition-all shadow-sm ${q.status === 'answered'
                    ? 'bg-emerald-50 border-emerald-200'
                    : 'bg-card border-border hover:border-primary/40 hover:shadow-md'
                    }`}>
                    <p className={`text-lg leading-relaxed font-medium ${q.status === 'answered' ? 'text-emerald-800' : 'text-foreground'
                      }`}>{q.content}</p>
